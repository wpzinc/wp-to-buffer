...
class WP_To_Social_Pro_Buffer_API {
		...
		<div class="wpzinc-option">
			<div class="full">
				<a href="<?php echo esc_attr( $this->get_oauth_url() ); ?>" class="button button-primary">
					<?php esc_html_e( 'Authorize Plugin', 'wp-to-buffer' ); ?>
				</a>
			</div>
		</div>
		<?php
	...
	private function parse_response( $response, $http_code, $params ) {
		...
		if ( is_null( $body ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Buffer API Error: HTTP Code %s. Sorry, we don\'t have any more information about this error. Please try again.', 'wp-to-buffer' ),
					$http_code
				)
			);
		}
		...
		switch ( $body->code ) {
			...
			case 403:
			case 1001:
				$message[] = __( 'Click the "Deauthorize Plugin" button, and then the "Authorize Plugin" button on the Plugin\'s Settings screen', 'wp-to-buffer' );
				break;
			 ...
			 */
			case 1003:
				$message[] = __( 'Run this Plugin on a publicly accessible domain that does not have password protection.', 'wp-to-buffer' );
				break;
			...
			case 1004:
				if ( strpos( $body->message, 'image' ) !== false ) {
					$message = array(
						__( 'A Featured Image is required for this status to be sent.', 'wp-to-buffer' ),
					);
				} else {
					$message = array(
						$body->message,
			 ...
			 */
			case 1025:
				$message[] = __( 'Change the status text using the Per-Post Settings, to ensure it is slightly different from the last status.', 'wp-to-buffer' );
				break;
		...
		return new WP_Error(
			...
			sprintf(
				...
				__( 'Buffer API Error: #%1$s: %2$s', 'wp-to-buffer' ),
				$body->code,
				implode( "\n", $message )
			)
		);
